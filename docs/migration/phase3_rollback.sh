#!/bin/bash
# PHASE 3 ROLLBACK: brings the legacy instructionscrafter.com site and its certificate back.
# Restores /etc/letsencrypt from the newest phase3 backup (returns the deleted certificate), copies the
# site file back and re-links it. Only meaningful while instructionscrafter.com still resolves to this VM.
set -u
...
LE=$(ls -t /root/letsencrypt-before-phase3-*.tgz 2>/dev/null | head -1)
S=$(ls -t /root/instruction-generator.site.before-phase3-* 2>/dev/null | head -1)
[ -n "$LE" ] && [ -n "$S" ] || { echo "backups not found in /root"; exit 1; }
tar xzf "$LE" -C / && echo "restored /etc/letsencrypt from $LE"
cp -a "$S" /etc/nginx/sites-available/instruction-generator
ln -sf /etc/nginx/sites-available/instruction-generator /etc/nginx/sites-enabled/instruction-generator
nginx -t || { echo "nginx -t FAILED; nothing reloaded"; exit 1; }
systemctl reload nginx && echo "legacy site re-enabled"
certbot certificates 2>/dev/null | grep -E 'Certificate Name|Expiry'
want instructionscrafter.com / 308
