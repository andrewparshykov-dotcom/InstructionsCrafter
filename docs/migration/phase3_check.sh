...
for f in $L $L.1; do
  [ -f "$f" ] && echo "$(basename $f) | $(grep -cE "$pat" $f) | $(grep -c 'instructionscrafter.com"' $f) | $(head -1 $f | cut -d'[' -f2 | cut -d']' -f1)"
done
for f in $L.*.gz; do
  [ -f "$f" ] && echo "$(basename $f) | $(zgrep -cE "$pat" $f) | $(zgrep -c 'instructionscrafter.com"' $f) | $(zcat $f | head -1 | cut -d'[' -f2 | cut -d']' -f1)"
done
echo
echo "most recent extension call to the legacy hostname (IP masked), if any:"
{ for f in $L.*.gz; do [ -f "$f" ] && zgrep -hE "$pat" "$f"; done; grep -hE "$pat" $L.1 $L 2>/dev/null; } | tail -1 | sed -E 's/^[0-9a-f.:]+/x.x.x.x/'
echo
