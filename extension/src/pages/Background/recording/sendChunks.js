...
export const sendChunks = async (override = false, target = null) => {
  ...
  try {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      ...
      console.debug("[InstructionsCrafter][BG] sendChunks: chunkCount check", {
        attempt,
        chunkCount,
      });
    ...
    if (chunkCount === 0) {
      console.warn(
        "[InstructionsCrafter][BG] sendChunks: no chunks available after waiting",
      );
      try {
    ...
    console.debug("[InstructionsCrafter][BG] sendChunks: collected chunks", {
      count: chunks.length,
    });
